...
func _process(_delta: float) -> void:
	if mouse_flight == null or player_cam == null:
		return
	update_graphics(_delta, mouse_flight)

func update_graphics(delta, controller: Node) -> void:
	...
	if mouse_pos:
		...
		dial.rotation = deg_to_rad(submarine.transform.basis.z.z) * 20
